...
function renderFortress(withExit) {
    return canvas => {
        ...
        write('Nossa fortaleza', canvas, 20, 20);
    };
}
...
const bufForest = renderBuf(SCREEN_WIDTH, SCREEN_HEIGHT, renderForest(PAL_FOREST_DARK, 'floresta negra'));
const bufForestLit = renderBuf(SCREEN_WIDTH, SCREEN_HEIGHT, renderForest(PAL_FOREST, 'Floresta razoavelmente ilumimada'));
const bufForestKegs = renderBuf(SCREEN_WIDTH, SCREEN_HEIGHT, canvas => {
    renderForest(PAL_FOREST, 'floresta da putaria')(canvas);
...
const bufWasteland = renderBuf(SCREEN_WIDTH, SCREEN_HEIGHT, renderWasteland('Deserto', 0));
const bufWastelandRoad = renderBuf(SCREEN_WIDTH, SCREEN_HEIGHT, renderWasteland('Estrada', 1));
const bufWastelandAperture = renderBuf(SCREEN_WIDTH, SCREEN_HEIGHT, renderWasteland('Area industrial', 2));
const bufTreasure = renderBuf(SCREEN_WIDTH, SCREEN_HEIGHT, canvas => {
    ...
    write('Mina de tesouro distante', canvas, 20, 20);
});
